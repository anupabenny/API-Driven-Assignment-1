...
def main_flow():
   ...
   data3 = run_task("chi_square.py", wait_for=[data2])
   data4 = run_task("CorrelationCoeffecient.py", wait_for=[data3])
   data5 = run_task("Encoding.py", wait_for=[data4])
   data6 = run_task("FeatureImportanceMLAlgorithms.py", wait_for=[data5])
   data7 = run_task("Normalization.py", wait_for=[data6]) 
   data8 = run_task("PearsonCorrelation.py", wait_for=[data7]) 
   data9 = run_task("Visualization.py", wait_for=[data8]) 

# To run 
if __name__ == "__main__":
    main_flow.serve(name="loan-workflow",
                      tags=["Loan Default datascience project workflow"],
                      parameters={},
